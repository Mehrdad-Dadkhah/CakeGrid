	...
	public function generate($data, $settings = array ()) {
		...
		foreach ($data as $i => $result) {
			...
			if ($element) {
				...
				$View = $this->_View;
				$content .= '<div class="manageActions">' . $View->element('CakeGrid.divGrid' . DS . 'column_actions', array( 'actions' => $this->__settings['actions'])) . '</div>';
			} elseif ($callback) {
			...
			if ($__addType) {
				...
				if ($type) {
					...
					$return .= '<' . $type .  $typeAttributes . ' class="TreeParent">';
				}
			}
			if ($indent) {
				$return .= "\r\n" . $whiteSpace . "\t";
			...
			if ($itemType) {
				...
				$return .= '<' . $itemType . $itemAttributes . ' class="treeChild">';
			}
			$return .= $content;
			/* Suffix */
			$__addType = false;
